...
Suppose we have $R(A, B, C, D)$ and decomposed relations $S_{1} = \pi_{AD}(R)$, $S_{1} = \pi_{AC}(R)$ and $S_{1} = \pi_{BCD}(R)$
...
Each row represents a decomposed relation!  If an attribute is not in a decomposed relation, we provide it a subscript for its row index.

Our goal is to use the FDs for $R$ to remove subscripts to obtain a row with no subscripts.  A row with no subscripts indicates the natural joins of each decomposition $S_{i}$ creates a row $t \in R$.  If we obtain $t$, then we have proven that the decomposition of $R$ has a lossless join!
...
### Chase Test Example 
__INSERT EXAMPLE AFTER QUIZ 7__

__GO THROUGH EXERCISES AND EXAMPLES AFTER THIS SECTION AFTER QUIZ 7__
