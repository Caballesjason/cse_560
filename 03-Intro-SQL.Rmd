...
```{sql FROM Naming Conflict Example, connection=conn}
...
```{sql Renaming Example, connection=conn}
...
For example:
```{sql Wildcard Example, connection=conn}
...
```{sql Explict variable naming, connection=conn}
...
```{sql self join example, connection=conn}
SELECT b1.course_id
  ,b2.course_id
...
```{sql UNION Example, connection=conn}
...
```{sql INTERSECTION Example, connection=conn}
...
```{sql EXCEPT Example, connection=conn}
