...
```{sql WHERE Example, connection=conn, max.print=5}
...
```{sql FROM Example, connection=conn, max.prin=5}
...
```{sql FROM Naming Conflict Example, connection=conn, max.print=5}
...
```{sql Renaming Example, connection=conn, max.print=5}
...
For example:
```{sql Wildcard Example, connection=conn, max.print=5}
...
```{sql Arithmetic Null Example, connection=conn}
-- NULL Arithmetic --
...
```{sql multiple relation example, eval=F}
...
```{sql Explict variable naming, connection=conn, max.print=5}
...
```{sql self join example, connection=conn, max.print=5}
SELECT b1.course_id
  ,b2.course_id
...
For example:
```{sql Subquery in FROM Example, eval=F}
...
```{sql UNION Example, connection=conn, max.print=5}
...
```{sql INTERSECTION Example, connection=conn, max.print=5}
...
```{sql EXCEPT Example, connection=conn, max.print=5}
...
```{sql Single-Tuple Query, eval=FALSE}
